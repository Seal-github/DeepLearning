...
# TODO 计算手臂两条直线的夹角
# 1. 计算斜率
def coordinate_to_slope(x1, y1, x2, y2):
    return (y2 - y1) / (x2 - x1)
# 2. 计算夹角
def angle_between_lines(m1, m2):
    ...
    Ly = np.sqrt(y.dot(y))
    # 根据向量之间求其夹角并四舍五入
    Angle = int((np.arccos(x.dot(y) / (float(Lx * Ly))) * 180 / np.pi) + 0.5)
    return Angle
...
def demo(args):
    ...
    camera = cv2.VideoCapture('./video/WeChat_20231110215318.mp4')
            ...
            for output in outputs:
                ...
                for kpt in reversed(kps_output):
                    ...
                    for i, sk in enumerate(skeleton):
                        ...
                        # TODO 获得两条直线的坐标表示用来计数夹角
                        # 在关键点中，9表示手肘，7表示肩膀，11表示手腕
                        if(sk == [7, 9] or sk == [9, 11]):
                    ...
                    # TODO 通过角度变化up-down来判断一次完整的俯卧撑
                    # 避免初期读取不到信息
                    if(line1 == ((0, 0), (0, 0)) or line2 == ((0, 0), (0, 0))):
                        continue
                    # 计算斜率
                    m1 = coordinate_to_slope(line1[0][0], line1[0][1], line1[1][0], line1[1][1])
                    m2 = coordinate_to_slope(line2[0][0], line2[0][1], line2[1][0], line2[1][1])
                    # 计算角度
                    angle = angle_between_lines(m1, m2)
                    # 当角度大于140度，认为是up
                    if(angle > 140):
                        stage = 1
                    # 当角度在60-100度并且之前状态是up,就计数加一
                    if(angle < 100 and angle>60 and stage == 1):
                        count += 1
                        # 把状态改为down
                        stage = 0
                    # 绘制计数数量
                    cv2.putText(frame, str(int(count)), (100, 100), cv2.FONT_HERSHEY_PLAIN, 5,(255, 0, 0), 5)
...
def main():
    ...
    with open('D:/Myworkplace/deep-learning-project/push-ups-Count/utils/args.yaml', errors='ignore') as f:
